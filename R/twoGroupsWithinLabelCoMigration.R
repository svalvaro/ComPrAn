...
#'
#' This function creates a scatter plot for a subset of proteins in dataFrame specified
#' in group1Data and group2Data, label states are always separated into facets
#' Intended use of the function - using scenario A data, compare shape of the migration profile
#' between a TWO GROUPS of proteins WITHIN the ONE LABEL STATE
...
#' @param group1Data character vector, contins list of Protein Group Accessions that belong to the group we want to plot for group 1
#' @param group1Name character, name that should be used for the group specified in group1Data
#' @param group2Data character vector, contins list of Protein Group Accessions that belong to the group we want to plot for group 2
#' @param group2Name character, name that should be used for the group specified in group2Data
...
#' @return plot
#' @export
twoGroupsWithinLabelCoMigration <- function(dataFrame,max_frac,group1Data = NULL, group1Name = 'group1', group2Data = NULL, group2Name ='group2',
  ...
  if(is.null(group1Data)|is.null(group2Data)) {
    stop('Please provide a list of group1 proteins and group2 proteins you would like to plot')
  }

  col_vector2 = c('#e6194B','#3cb44b','#ffe119','#4363d8','#f58231','#911eb4','#42d4f4','#f032e6','#bfef45','#fabebe','#469990','#e6beff','#9A6324','#fffac8','#800000','#aaffc3','#808000','#ffd8b1','#000075','#a9a9a9','#ffffff','#000000')
  ...
  group1Data <- data.frame(protein = group1Data, group = rep(group1Name,length(group1Data)), stringsAsFactors=FALSE)
  group2Data <- data.frame(protein = group2Data, group = rep(group2Name,length(group2Data)), stringsAsFactors=FALSE)
  ...
  if(showTitle) {
    p <- p +
      labs(title = paste(group1Name, ' and ', group2Name, sep = ''))
  }
