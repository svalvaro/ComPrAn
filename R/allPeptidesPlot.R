...
#' 
#' @param env: environtmnt, environtment containing data frames of peptides for each protein
#'       indexed by `Protein Group Accessions`
#' @param dataFrame: character, `Protein Group Accession` to show in the plot
...
allPeptidesPlot <- function(env, dataFrame, max_frac, 
  ...
  dataFrame %>% 
    ...
    mutate (repPepValue = ifelse(all(is.na(`Precursor Area`)),NA,mean(`Precursor Area`[repPepA], na.rm=T))) %>% 
    ungroup() -> dataFrame
  ...
  if (titleLabel == 'all'){
    p <- p + labs(title = str_remove(str_extract(description, "^.* OS"), " OS"),
                  ...
                  caption = paste('UniProt ID:', proteinName, sep ='')) +
  ...
  #add rep pep line
  if (repPepLine) { ## add line that goes throug values of chosen representative peptide
    p <- p + geom_line(aes(y = repPepValue, colour = isLabel, linetype = 'representative peptide'),size=1, na.rm = T)+
      geom_point(aes(y = repPepValue, colour = isLabel),na.rm = T, alpha = 1)+
      scale_linetype_manual('Line type', values = linetype_vector) 
  }
  ...
  if(separateLabStates){
    p <- p + facet_wrap(isLabel ~ ., nrow =2,scales = "free_x")+theme(strip.text = element_blank())
  }
